  ...
  <main id="main" :style="{ backgroundImage: `url('${image}')` }">
    ...
    <transition name="shake" mode="out-in">
      <section :key="botText" class="bot-profile" aria-live="polite">
        <img
          ...
          :src="botImage"
        />
        <img v-else class="top-head-icon" src="/img/icon.png" :alt="botName" />
        <div class="bot-title">
          <span>{{ botName }}</span>
        </div>
      </section>
    ...
    <transition name="shake" mode="out-in">
      <section :key="meText" class="my-profile" aria-live="polite">
        <img class="top-head-icon" :src="myAvatar" alt="My-Avatar" />
        <div class="bot-title">
          <span
            ><button
              ...
              @click="openAvatar"
            >
              <i class="material-icons" aria-hidden="true">face</i></button
            >Me</span
          >
        </div>
      </section>
...
export default {
  ...
  beforeMount() {
    ...
    this.image =
      ...
      '/img/backgrounds/Wintery-Sunburst.svg'
    this.myAvatar =
      localStorage.getItem('avatar') || '/img/avatars/SVG/flat/27-ninja.svg'
  ...
  methods: {
    ...
    handle(response) {
      ...
      if (response.queryResult.sentimentAnalysisResult) {
        ...
        if (this.botDetails) {
          const score = parseFloat(response.queryResult.sentimentAnalysisResult.queryTextSentiment
              .score *
          ...
          if (score >= 0.6) {
            ...
            this.botImage =
              this.botDetails.sentiment_v_positive || this.botImage
            return
          }
          // 0.1-0.6 = positive
          if (score > 0.2) {
