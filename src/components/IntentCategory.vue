<template>
  <div
    class="bg-yellow-100 border-l-4 border-purple-400 rounded px-3"
    :class="category.dirty ? 'dirty' : ''"
  >
    <div class="flex items-center justify-between my-4">
      <h2 @click="() => (category.edit = !category.edit)">
        <i class="material-icons" aria-hidden="true">{{
          category.edit ? 'expand_less' : 'expand_more'
        }}</i>
        {{ category.name }}
        <span class="tooltip"
      ...
      <div>
        <button @click="() => $emit('add-new', category)">
          Add New "{{ category.name }}" item<i
    ...
    <div v-if="category.edit" class="item-details">
      ...
      />
      <IntentItem
        v-for="subintent in filteredList"
        :key="subintent.id"
